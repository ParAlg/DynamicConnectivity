...
template<typename Derived>
bool ElementBase<Derived>::CASNext(int level, Derived* old_next, Derived* new_next) {
  return CAS(&neighbors_[level].next, old_next, new_next);
}

template<typename Derived>
bool ElementBase<Derived>::CASPrev(int level, Derived* old_prev, Derived* new_prev) {
...
Derived* ElementBase<Derived>::FindRepresentative() const {
  ...
  while (current_element->neighbors_[current_level].next != nullptr && seen_element != current_element) {
...
void ElementBase<Derived>::Join(Derived* left, Derived* right) {
  ...
  while (left != nullptr && right != nullptr) {
    if (left->neighbors_[level].next == nullptr && left->CASNext(level, nullptr, right)) {
