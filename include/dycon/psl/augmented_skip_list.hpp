...
struct DefaultGetter {
  ...
  static inline T& Get(T& elem) { return elem; }
...
void AugmentedElementBase<D, F>::UpdateTopDownImpl(int level, D* curr, bool is_loop_start) {
  while (is_loop_start || (curr != nullptr && curr->height_ < level + 1)) {
    if (curr->update_level_ != _internal::NA && curr->update_level_ < level) {
      parlay::par_do([&]() { curr->template UpdateTopDown<Getter>(level - 1); },
...
void AugmentedElementBase<D, F>::BatchJoin(const parlay::sequence<std::pair<D*, D*>>& joins) {
  ...
  parlay::parallel_for(0, len, [&](const size_t i) { Join(joins[i].first, joins[i].second); });
...
void AugmentedElementBase<D, F>::BatchSplit(const parlay::sequence<D*>& splits) {
  ...
  parlay::parallel_for(0, len, [&](const size_t i) { splits[i]->Split(); });
  parlay::parallel_for(0, len, [&](const size_t i) {
    ...
    // update.
    bool can_proceed{curr->update_level_ == _internal::NA && CAS<int8_t>(&curr->update_level_, _internal::NA, 0)};
    if (can_proceed) {
  ...
  parlay::parallel_for(0, len, [&](const size_t i) { splits[i]->update_level_ = _internal::NA; });
}
