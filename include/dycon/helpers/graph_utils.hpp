...
struct graph_utils {
  ...
  static graph transpose(const graph& G) {
    auto pairs = flatten(parlay::delayed::tabulate(G.size(), [&](vertex i) {
      return map(
          G[i], [=](auto ngh) { return std::pair(ngh, i); }, 1000);
    }));
  ...
  static graph symmetrize(const graph& G) {
    ...
    return parlay::tabulate(G.size(), [&](long i) {
      return parlay::filter(parlay::remove_duplicates(parlay::append(GT[i], G[i])), [=](auto j) { return i != j; });
    });
  }
  ...
  static graph symmetrize(const edges& Ein, long n) {
    ...
    return parlay::tabulate(n, [&](long i) { return parlay::remove_duplicates(append(G[i], GT[i])); });
  }

  static edges to_edges(const graph& G) {
    return flatten(
        parlay::tabulate(G.size(), [&](vertex u) { return map(G[u], [=](vertex v) { return std::pair(u, v); }); }));
  }
  ...
  static long num_vertices(const edges& E) {
    ...
    return reduce(map(E, [](auto e) { return std::max(e.first, e.second); }), parlay::maximum<vertex>()) + 1;
  }
  ...
  static sparse_matrix to_normalized_matrix(const graph& G) {
    ...
    return parlay::map(G, [&](auto& nghs) {
      return parlay::map(
          nghs, [&](vertex c) { return element(c, 1.0 / column_counts[c]); }, 100);
    });
  }
  ...
  static graph grid_graph(long n) {
    ...
    return parlay::tabulate(sqn * sqn, [&](vertex u) {
      ...
      return filter(nghs, [=](vertex v) { return (v >= 0 && v < sqn * sqn && abs(u % sqn - v % sqn) < 2); });
    });
  }
  ...
  static void print_graph_stats(const edges& E, long n) {
    ...
    long max_degree = parlay::reduce(parlay::histogram_by_index<long>(e1, n), parlay::maximum<long>());
  ...
  static graph read_graph_from_file_pbbs(const std::string& filename) {
    ...
    auto offsets = parlay::tabulate(n, [&](long i) { return parlay::chars_to_double(tokens[i + 3]); });
    auto edges = parlay::tabulate(m, [&](long i) { return (vertex)parlay::chars_to_double(tokens[i + n + 3]); });
  ...
  static graph read_graph_from_file(const std::string& filename) {
    ...
    auto lengths = parlay::delayed::tabulate(n, [&](long i) { return parlay::chars_to_double(tokens[i + 2]); });
    auto edges = parlay::tabulate(m, [&](long i) { return (vertex)parlay::chars_to_double(tokens[i + n + 2]); });
    auto [offsets, total] = scan(lengths);
    return parlay::tabulate(
        n, [&, o = offsets.begin()](vertex i) { return scan_inclusive(edges.cut(o[i], o[i] + lengths[i])); });
  }
  ...
  static graph read_symmetric_graph_from_file(const std::string& filename) {
    ...
    auto GT = transpose(G);
    return parlay::tabulate(G.size(), [&](long i) { return parlay::append(parlay::sort(GT[i]), G[i]); });
  }
  ...
  static void write_graph_to_file(const graph& G, const std::string& filename) {
    ...
    auto edges = flatten(parlay::tabulate(G.size(), [&](long i) {
      auto nghs = sort(G[i]);
      return parlay::tabulate(nghs.size(), [&](long j) -> long { return j == 0 ? nghs[0] : nghs[j] - nghs[j - 1]; });
    }));
    auto all = flatten(parlay::sequence<lseq>({lseq(1, (long)G.size()), lseq(1, (long)edges.size()), lengths, edges}));
    auto newline = parlay::chars(1, '\n');
    chars_to_file(flatten(map(all, [&](long v) { return append(parlay::to_chars(v), newline); })), filename);
  }

  static void write_symmetric_graph_to_file(const graph& G, const std::string& filename) {
    auto GR =
        parlay::tabulate(G.size(), [&](vertex u) { return parlay::filter(G[u], [&](vertex v) { return v > u; }); });
  ...
  static void write_edges_to_filename(const edges& E, const std::string& filename, vertex& n, vertex& m) {
    std::ofstream fout;
    fout.open(filename);
    fout << n << " " << m << std::endl;
  ...
  static parlay::sequence<vertex> avail_vertices(const edges& E, vertex n) {
    ...
    return parlay::filter(parlay::iota<vertex>(n), [&](auto i) { return flags[i]; });
  }
  static parlay::sequence<queries> generate_CC_queries(size_t& num_batches, parlay::sequence<edges>& E, size_t& n,
                                                       size_t& num_queries) {
      ...
      parlay::parallel_for(0, Q.size(), [&](size_t j) {
        Q[j].first =  // 50 percent non-connect pair
            j % 2 ? E_[parlay::hash64(j) % E_.size()].first
                  : candidate_vertices[parlay::hash64(j + candidate_vertices.size()) % candidate_vertices.size()];
        Q[j].second =  // 50 percent non-connect pair
            j % 2 ? E_[parlay::hash64(j) % E_.size()].second
                  : candidate_vertices[parlay::hash64(j) % candidate_vertices.size()];
      });
      return Q;
    });
  }
