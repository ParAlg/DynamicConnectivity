...
struct union_find {
  ...
  union_find(vertex n) : parents(parlay::tabulate<vertex>(n, [&](vertex i) { return i; })) {}
  vertex find(vertex x) { return parents[x] == x ? x : (parents[x] = find(parents[x])); }
...
inline void static_connectivity(parlay::sequence<edge>& E, parlay::sequence<vertex>& V, size_t& n, size_t& m,
                                parlay::sequence<query>& Q, parlay::sequence<bool>& Ans) {
