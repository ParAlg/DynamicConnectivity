  ...
  while (Cu->getSize() + Cv->getSize() <= (1 << (p->getLevel() - 1))) {
    ...
    if (Cu->getLevel() == p->getLevel() - 1 || Cv->getLevel() == p->getLevel() - 1) {
      if (Cu->getLevel() == Cv->getLevel()) {  // Merge two nodes directly under parent
        ...
        Cu = *iu;
        Cv = *iv;
      } else if (Cu->getLevel() > Cv->getLevel()) {  // If one node directly under parent add
                                                     // other as child
        if (Cu->getSize() + Cv->getSize() > (1 << Cu->getLevel())) break;
        localTree::deleteFromParent(Cu);
        localTree::deleteFromParent(Cv);
        localTree::addChild(Cu, Cv);
        localTree::addChild(p, Cu);
      ...
      if (std::get<0>(eu) == true) {
        ...
        if (Hv.find(Cuv) != Hv.end()) {
          if (Eu.empty()) {
            ...
            for (auto it : Ru) {
              _v += it->getSize();
              if (it->getLevel() > C->getLevel()) C->setLevel(it->getLevel());
            }
            C->setLevel(std::max(C->getLevel(), (size_t)std::ceil(std::log2(_v))));
            for (auto it : Ru) {
        ...
        if (Eu.empty()) {
          ...
          for (auto it : Ru) {
            _v += it->getSize();
            if (it->getLevel() > _CP->getLevel()) _CP->setLevel(it->getLevel());
          }
          _CP->setLevel(std::max(_CP->getLevel(), (size_t)std::ceil(std::log2(_v))));
          for (auto it : Ru) {
