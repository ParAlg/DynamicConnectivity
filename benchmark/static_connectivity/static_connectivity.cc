...
using namespace std;
void test(parlay::sequence<edges> &instances, parlay::sequence<queries> &q, size_t num_batches, ofstream &faq) {
...
int main(int argc, char **argv) {
  ...
  vertex m = E.size();
  // cout << n << " " << m << " -b " << num_batches << " -q " << num_queries << endl;
  ...
  auto instances_ins = parlay::tabulate(
      num_batches, [&](size_t i) { return parlay::to_sequence(E.cut(batch_size[0], batch_size[i + 1])); });
  ...
  auto instances_del = parlay::tabulate(
      num_batches, [&](size_t i) { return parlay::to_sequence(E_.cut(batch_size[i + 1], batch_size[num_batches])); });
