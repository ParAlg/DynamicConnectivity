...
void bench_uncompressed_CWN_root(edges &instance,size_t num_batches, size_t n,
                                 std::string ansfile,
  ...
  t.next("\npreparing for benchmarking uncompressed CWN with edges inserted to root");
  for (size_t i = 0; i < instance.size();i++)
    F.insertToRoot(instance[i].first, instance[i].second);
  t.next("start benchmarking");
...
void bench_uncompressed_CWN_blocked(edges &instance,size_t num_batches, size_t n,
                                    std::string ansfile,
                                    parlay::sequence<edges> &batches_ins,
                                    parlay::sequence<queries> &queries_ins,
                                    parlay::sequence<edges> &batches_del,
  ...
  CWN F(n);
  F.lmax = std::ceil(std::log2(n));
  t.next("\npreparing for benchmarking uncompressed CWN with edges inserted to block");
  for (size_t i = 0; i < instance.size();i++)
    F.insertToBlock(instance[i].first, instance[i].second);
  t.next("start benchmarking");
  for (size_t i = 0; i < num_batches; i++) {
    for (size_t j = 0; j < batches_ins[i].size(); j++) {
      long u = batches_ins[i][j].first;
...
void bench_compress_CWN_root(edges &instance,size_t num_batches, size_t n, std::string ansfile,
                             parlay::sequence<edges> &batches_ins,
                             parlay::sequence<queries> &queries_ins,
                             parlay::sequence<edges> &batches_del,
                             parlay::sequence<queries> &queries_del) {
  ...
  SCCWN F(n);
  F.lmax = std::ceil(std::log2(n));
  t.next("\npreparing for benchmarking comressed CWN with edges inserted to root");
  for (size_t i = 0; i < instance.size();i++)
    F.insertToRoot(instance[i].first, instance[i].second);
  t.next("start benchmarking");
  for (size_t i = 0; i < num_batches; i++) {
...
void bench_compress_CWN_lca(edges &instance,size_t num_batches, size_t n, std::string ansfile,
                            parlay::sequence<edges> &batches_ins,
                            parlay::sequence<queries> &queries_ins,
                            parlay::sequence<edges> &batches_del,
                            parlay::sequence<queries> &queries_del) {
  ...
  SCCWN F(n);
  F.lmax = std::ceil(std::log2(n));
  t.next("\npreparing for benchmarking comressed CWN with edges inserted to lca");
  for (size_t i = 0; i < instance.size();i++)
    F.insertToLCA(instance[i].first, instance[i].second);
  t.next("start benchmarking");
  for (size_t i = 0; i < num_batches; i++) {
    for (size_t j = 0; j < batches_ins[i].size(); j++) {
      long u = batches_ins[i][j].first;
...
void bench_compress_CWN_blocked(edges &instance,size_t num_batches, size_t n,
                                std::string ansfile,
                                parlay::sequence<edges> &batches_ins,
                                parlay::sequence<queries> &queries_ins,
                                parlay::sequence<edges> &batches_del,
  ...
  SCCWN F(n);
  F.lmax = std::ceil(std::log2(n));
  t.next("\npreparing for benchmarking comressed CWN with edges inserted to blocked");
  for (size_t i = 0; i < instance.size();i++)
    F.insertToBlock(instance[i].first, instance[i].second);
  t.next("start benchmarking");
  for (size_t i = 0; i < num_batches; i++) {
...
void bench_seq_hdt(edges &instance,size_t num_batches, size_t n, std::string ansfile,
                   parlay::sequence<edges> &batches_ins,
                   parlay::sequence<queries> &queries_ins,
                   parlay::sequence<edges> &batches_del,
                   parlay::sequence<queries> &queries_del) {
  ...
  t.next("\npreparing for benchmarking sequential hdt");
  for (size_t i = 0; i < instance.size();i++){
    long u = instance[i].first;
    long v = instance[i].second;
...
int main(int argc, char **argv) {
  ...
  std::string usage = "Usage: Connectivity [-a #algorithms] [-b #batches] [-s batch size] [-q "
  ...
  size_t batch_size = P.getOptionIntValue("-s", 1000);
  std::cout << "#batches = " <<num_batches << " batch size = " << batch_size << std::endl;
  ...
  auto batch_cut = parlay::tabulate(
      num_batches + 1, [&](size_t i) { return m - batch_size*num_batches + i * batch_size;  });
  
  E = parlay::random_shuffle(E);

  auto instance = parlay::to_sequence(E.cut(0,E.size()-num_batches*batch_size));
  auto batches_ins = parlay::tabulate(num_batches, [&](size_t i) {
    return parlay::to_sequence(E.cut(batch_cut[i], batch_cut[i + 1]));
  });
  auto queries_ins =
  ...
  auto batches_del = parlay::tabulate(num_batches, [&](size_t i) {
    return parlay::to_sequence(E_.cut(batch_cut[i], batch_cut[i + 1]));
  });
  auto queries_del =
  ...
  parlay::execute_with_scheduler(1, [&]() {
    ...
    case 0:
      ...
      //     instance,num_batches, n, Out + "_uncompressed_CWN_root.ans", batches_ins,
      //     queries_ins, batches_del, queries_del);
      // bench_uncompressed_CWN_blocked(
      //     instance,num_batches, n, Out + "_uncompressed_CWN_blocked.ans", batches_ins,
      //     queries_ins, batches_del, queries_del);
      // bench_compress_CWN_root(instance,num_batches, n, Out + "_compressed_CWN_root.ans",
      //                         batches_ins, queries_ins, batches_del,
      //                         queries_del);
      bench_compress_CWN_lca(instance,num_batches, n, Out + "_compressed_CWN_lca.ans",
                             batches_ins, queries_ins, batches_del,
                             queries_del);
      // bench_compress_CWN_blocked(
      //     instance,num_batches, n, Out + "_compressed_CWN_blocked.ans", batches_ins,
      //     queries_ins, batches_del, queries_del);
      bench_seq_hdt(instance,num_batches, n, Out + "_seqhdt.ans", batches_ins,
                    queries_ins, batches_del, queries_del);
      break;
    case 1:
      bench_uncompressed_CWN_root(
