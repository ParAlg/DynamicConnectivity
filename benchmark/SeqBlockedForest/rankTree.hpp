...
class rankTree {
 ...
 private:
  ...
  static size_t getLeafSize(rankTree* r);
  ...
  static localTree* getLTreeTest();
  static localTree* getSmallest(arr& A);
  static localTree* getGreatest(arr& A);
...
inline rankTree::rankTree(rankTree* T1, rankTree* T2) {
  ...
  this->min_leaf = getLeafSize(T1->min_leaf) < getLeafSize(T2->min_leaf) ? T1->min_leaf : T2->min_leaf;
  this->max_leaf = getLeafSize(T1->max_leaf) > getLeafSize(T2->max_leaf) ? T1->max_leaf : T2->max_leaf;
  this->lchild = T1;
  this->rchild = T2;
  T1->parent = this;
  T2->parent = this;
...
inline localTree* rankTree::getSmallest(arr& A) {
  auto smallest = A[0]->min_leaf;
  for (auto it : A)
    if (getLeafSize(smallest) > getLeafSize(it->min_leaf)) smallest = it->min_leaf;
  return smallest->descendant;
}
inline localTree* rankTree::getGreatest(arr& A) {
  auto greatest = A[0]->max_leaf;
  for (auto it : A)
    if (getLeafSize(greatest) < getLeafSize(it->max_leaf)) greatest = it->max_leaf;
  return greatest->descendant;
}
...
inline rankTree::arr rankTree::testRankTreesGen(size_t n) {
  ...
  parlay::parallel_for(0, n,
                       [&](size_t i) { A[i] = new rankTree(parlay::hash64(i) % parlay::log2_up(n), nullptr, p, 0); });
  A = testSortByRank(A);
